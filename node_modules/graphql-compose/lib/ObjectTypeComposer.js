...
var _createThunkedObjectProxy = require("./utils/createThunkedObjectProxy");
...
class ObjectTypeComposer {
  ...
  static createTemp(typeDef, schemaComposer) {
    ...
    if ((0, _is.isString)(typeDef)) {
      ...
      if ((0, _is.isFunction)(fields)) {
        ...
        TC.addFields((0, _configToDefine.convertObjectFieldMapToConfig)(fields, sc));
      } else if ((0, _is.isObject)(fields)) {
        TC.addFields(fields);
      }

      const interfaces = typeDef.interfaces;
      if (Array.isArray(interfaces)) TC.setInterfaces(interfaces);else if ((0, _is.isFunction)(interfaces)) {
        // rewrap interfaces `() => [i1, i2]` -> `[()=>i1, ()=>i2]`
        // helps to solve hoisting problems
        TC.setInterfaces((0, _configToDefine.convertInterfaceArrayAsThunk)(interfaces, sc));
      }
  ...
  constructor(graphqlType, schemaComposer) {
    ...
    if (_graphqlVersion.graphqlVersion >= 14) {
      ...
      this._gqcInterfaces = (0, _configToDefine.convertInterfaceArrayAsThunk)(this._gqType._interfaces || this._gqType._typeConfig.interfaces, this.schemaComposer);
  ...
  setField(fieldName, fieldConfig) {
    this._gqcFields[fieldName] = this.schemaComposer.typeMapper.convertOutputFieldConfig(fieldConfig);
    return this;
  }
  /**
  ...
  addFields(newFields) {
    this._gqcFields = _objectSpread({}, this._gqcFields, {}, this.schemaComposer.typeMapper.convertOutputFieldConfigMap(newFields));
    return this;
  }
  /**
    ...
    Object.keys(newFields).forEach(fieldName => {
      ...
      if (names.length === 0) {
        ...
        if (!this.hasField(name)) {
          childTC = ObjectTypeComposer.create(`${this.getTypeName()}${(0, _misc.upperFirst)(name)}`, this.schemaComposer);
          this.setField(name, {
            type: childTC,
            resolve: () => ({})
          });
    ...
    fieldNames.forEach(fieldName => {
      ...
      if (names.length === 0) {
        ...
        if (this.hasField(name)) {
          ...
          if (subTC instanceof ObjectTypeComposer || subTC instanceof _EnumTypeComposer.EnumTypeComposer) {
  ...
  getFieldConfig(fieldName) {
    ...
    return _objectSpread({
      type: type.getType(),
      args: args && (0, _misc.mapEachKey)(args, ac => _objectSpread({}, ac, {
  ...
  addFieldArgs(fieldName, newArgs) {
    const fc = this.getField(fieldName);
    fc.args = _objectSpread({}, fc.args, {}, this.schemaComposer.typeMapper.convertArgConfigMap(newArgs, fieldName, this.getTypeName()));
    return this;
  }
  ...
  getType() {
    this._gqType.astNode = (0, _definitionNode.getObjectTypeDefinitionNode)(this);
  ...
  clone(newTypeNameOrTC) {
    ...
    cloned._gqcFields = (0, _misc.mapEachKey)(this._gqcFields, fieldConfig => _objectSpread({}, fieldConfig, {
      args: (0, _misc.mapEachKey)(fieldConfig.args, argConfig => _objectSpread({}, argConfig, {
    ...
    this.getResolvers().forEach(resolver => {
      const newResolver = resolver.clone(); // in clonned resolvers we also replace cloned ObjectTypeComposer

      newResolver.type = (0, _typeHelpers.replaceTC)(newResolver.type, tc => {
  ...
  merge(type) {
    if (type instanceof _graphql.GraphQLObjectType) {
      ...
      const tmp = _InterfaceTypeComposer.InterfaceTypeComposer.createTemp(type, this.schemaComposer);

      this.addFields(tmp.getFields());
    } else if (type instanceof ObjectTypeComposer) {
      this.addFields(type.getFields());
      this.addInterfaces(type.getInterfaces()); // Feel free to add other properties for merging two TypeComposers.
  ...
  addRelation(fieldName, opts) {
    ...
    if (opts.hasOwnProperty('resolver')) {
      if ((0, _is.isFunction)(opts.resolver)) {
        this._gqcFields[fieldName] = (0, _createThunkedObjectProxy.createThunkedObjectProxy)(() => this._relationWithResolverToFC(opts, fieldName));
      } else {
  ...
  _relationWithResolverToFC(opts, fieldName = '') {
    ...
    return {
      ...
      extensions: _objectSpread({}, resolver.extensions, {}, opts.extensions)
    };
  }
  ...
  getNestedTCs(opts = {}, passedTypes = new Set()) {
    ...
    this.getFieldNames().forEach(fieldName => {
      ...
      if (!passedTypes.has(tc) && !exclude.includes(tc.getTypeName())) {
        ...
        if (tc instanceof ObjectTypeComposer || tc instanceof _UnionTypeComposer.UnionTypeComposer) {
  ...
  toSDL(opts) {
    ...
    if (deep) {
      let r = '';
      r += (0, _schemaPrinter.printObject)(this.getType(), innerOpts);
    ...
    return (0, _schemaPrinter.printObject)(this.getType(), innerOpts);
  }
