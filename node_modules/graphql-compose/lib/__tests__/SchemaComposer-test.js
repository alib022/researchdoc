...
describe('SchemaComposer', () => {
  ...
  describe('constructor', () => {
    describe('if GraphQLSchema provided', () => {
      const gqUser = new _graphql.GraphQLObjectType({
        name: 'User',
        fields: () => ({
          name: {
      ...
      const gqPost = new _graphql.GraphQLObjectType({
        name: 'Post',
        fields: () => ({
          title: {
            type: _graphql.GraphQLString
      ...
      const gqSchema = new _graphql.GraphQLSchema({
        query: new _graphql.GraphQLObjectType({
          name: 'OoopsQuery',
          fields: () => ({
            post: {
              type: new _graphql.GraphQLList(gqPost)
            },
            user: {
              type: gqUser
            }
  ...
  describe('buildSchema()', () => {
    ...
    it('should provide proper Schema when provided only Query', async () => {
      ...
      const schema = sc.buildSchema();
      expect((await (0, _graphql.graphql)({
        schema,
        source: `
            query {
              num
  ...
  describe('addResolveMethods', () => {
    it('should add resolve methods to fields in graphql-tools way', async () => {
      ...
      const schema = sc.buildSchema();
      expect((await (0, _graphql.graphql)(schema, '{ posts { id title votes } }'))).toEqual({
        data: {
          posts: [{
            id: 1,
            title: 'Post title',
  ...
  describe('clone()', () => {
    ...
    it('directives should be different', () => {
      const directive = new _graphql.GraphQLDirective({
        name: 'D',
        locations: ['QUERY']
      });
      ...
      const directive2 = new _graphql.GraphQLDirective({
        name: 'D2',
        locations: ['QUERY']
      });
